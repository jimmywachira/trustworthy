...
<article class="overflow-hidden rounded-2xl bg-white/92 shadow-premium ring-1 ring-slate-200/90 backdrop-blur-sm transition hover:-translate-y-0.5 hover:shadow-[0_22px_48px_-30px_rgb(15_23_42/0.45)] lg:bg-white/86 dark:bg-slate-900/92 dark:ring-slate-800/95 dark:hover:ring-slate-700">
    <div class="relative">
        ...
        <x-price :amount="$property->price" :period="$property->type === 'rent' ? 'month' : null" class="absolute left-3 top-3 rounded-full bg-emerald-600 px-3 py-1.5 text-sm font-semibold text-white shadow-sm sm:left-4 sm:top-4 sm:px-4" />

        <div class="absolute right-3 top-3 space-y-1.5 sm:right-4 sm:top-4 sm:space-y-2">
            ...
            @auth
                <form method="POST" action="{{ route('saved-properties.toggle', $property) }}">
                    @csrf
                    <button type="submit" class="rounded-full px-3 py-1.5 text-xs font-semibold uppercase tracking-wide backdrop-blur transition sm:py-1 {{ $isSaved ? 'bg-emerald-600 text-white dark:bg-emerald-500' : 'bg-white/92 text-slate-700 hover:bg-white dark:bg-slate-900/92 dark:text-slate-200 dark:hover:bg-slate-800' }}">
                        {{ $isSaved ? 'Saved' : 'Save' }}
                    </button>
                </form>
            @else
                <a href="{{ route('login') }}" class="inline-flex rounded-full bg-white/92 px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-slate-700 backdrop-blur transition sm:py-1 hover:bg-white dark:bg-slate-900/92 dark:text-slate-200 dark:hover:bg-slate-800">
                    Save
                </a>
            @endauth
        </div>
    ...
    <div class="space-y-3.5 p-4 sm:space-y-4 sm:p-5 2xl:p-6">
        <div>
        ...
        <div class="grid grid-cols-3 gap-2 rounded-xl bg-slate-50/90 p-2.5 text-sm text-slate-700 sm:p-3 dark:bg-slate-950/85 dark:text-slate-300">
        ...
        <a
            ...
            class="inline-flex w-full items-center justify-center rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white transition sm:py-2.5 2xl:py-3 hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-2 dark:bg-emerald-500 dark:hover:bg-emerald-400 dark:focus:ring-emerald-400 dark:focus:ring-offset-slate-900"
        >
            Request Viewing
        </a>
    </div>
