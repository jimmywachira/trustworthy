            ...
            <aside class="hidden w-72 flex-col border-r border-slate-200 bg-white p-5 lg:flex">
                ...
                <nav class="space-y-1 text-sm">
                    ...
                    @if (auth()->user()->hasPermission('manage_admins'))
                        <a href="{{ route('admin.users.index') }}" class="block rounded-lg px-3 py-2 {{ request()->routeIs('admin.users.*') ? 'bg-slate-900 text-white' : 'text-slate-700 hover:bg-slate-100' }}">Users & Permissions</a>
                    @endif
                </nav>
