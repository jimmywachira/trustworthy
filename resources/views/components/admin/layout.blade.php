...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="relative min-h-screen bg-slate-50 text-slate-900 antialiased dark:bg-slate-950 dark:text-slate-100">
        ...
        <div class="relative z-10 flex min-h-screen">
            <aside class="hidden w-72 flex-col border-r border-slate-200 bg-white p-5 lg:flex">
                ...
                <nav class="space-y-1 text-sm">
                    ...
                    @if (auth()->user()->hasPermission('manage_appointments'))
                        <a href="{{ route('admin.appointments.index') }}" class="inline-flex w-full items-center gap-2 rounded-lg px-3 py-2 {{ request()->routeIs('admin.appointments.*') ? 'bg-slate-900 text-white' : 'text-slate-700 hover:bg-slate-100' }}"><ion-icon name="calendar-outline" class="h-4 w-4" aria-hidden="true"></ion-icon>Appointments</a>
                    @endif
                    ...
                    @if (auth()->user()->hasPermission('manage_admins'))
                        <a href="{{ route('admin.users.index') }}" class="inline-flex w-full items-center gap-2 rounded-lg px-3 py-2 {{ request()->routeIs('admin.users.*') ? 'bg-slate-900 text-white' : 'text-slate-700 hover:bg-slate-100' }}"><ion-icon name="people-circle-outline" class="h-4 w-4" aria-hidden="true"></ion-icon>Users & Permissions</a>
                    @endif
                </nav>
