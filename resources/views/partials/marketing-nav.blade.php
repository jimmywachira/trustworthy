<header x-data="{ mobileOpen: false }" class="sticky top-0 z-40 border-b border-slate-200/80 bg-white/88 shadow-sm backdrop-blur-lg transition-colors lg:bg-white/78 lg:backdrop-blur-xl dark:border-slate-800/90 dark:bg-slate-950/90 dark:lg:bg-slate-950/82">
    <div class="mx-auto max-w-7xl px-4 py-2.5 sm:px-6 sm:py-3 md:py-3.5 lg:px-8 lg:py-3.5 2xl:py-4">
        <div class="flex items-center gap-2.5 md:gap-3">
            ...
            <div class="ml-auto flex items-center gap-2 sm:gap-2.5 lg:gap-3">
                <x-theme-toggle />

                <div class="hidden lg:flex lg:items-center lg:gap-3">
                    @auth
                        @if (auth()->user()->is_admin)
                            <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center rounded-full bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition xl:px-4.5 xl:py-2.5 hover:bg-emerald-500 dark:bg-emerald-500 dark:hover:bg-emerald-400">Admin Panel</a>
                        @else
                            <a href="{{ route('dashboard') }}" class="inline-flex items-center rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-sm transition xl:px-4.5 xl:py-2.5 hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-200">Dashboard</a>
                        @endif
                    @else
                        <a href="{{ route('login') }}" class="inline-flex items-center rounded-full bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition xl:px-4.5 xl:py-2.5 hover:bg-emerald-500 dark:bg-emerald-500 dark:hover:bg-emerald-400">Sign In</a>
                    @endauth
                </div>
                ...
                <button
                    ...
                    class="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-slate-300 text-slate-700 transition sm:h-10 sm:w-10 hover:bg-slate-100 lg:hidden dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
                >
        ...
        <div
            ...
            class="mt-2.5 rounded-2xl border border-slate-200/90 bg-white/95 p-2.5 shadow-premium backdrop-blur-md sm:mt-3 sm:p-3 md:p-3.5 lg:hidden dark:border-slate-800/90 dark:bg-slate-900/95"
        >
            <nav class="grid gap-1.5">
                <a @click="mobileOpen = false" href="{{ route('properties.index', ['type' => 'sale']) }}" class="inline-flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium transition {{ request()->routeIs('properties.index') && request()->query('type') === 'sale' ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800' }}"><ion-icon name="trending-up-outline" class="h-4 w-4" aria-hidden="true"></ion-icon>Buy</a>
                <a @click="mobileOpen = false" href="{{ route('properties.index', ['type' => 'rent']) }}" class="inline-flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium transition {{ request()->routeIs('properties.index') && request()->query('type') === 'rent' ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800' }}"><ion-icon name="key-outline" class="h-4 w-4" aria-hidden="true"></ion-icon>Rent</a>
                <a @click="mobileOpen = false" href="{{ route('sell') }}" class="inline-flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium transition {{ request()->routeIs('sell') ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800' }}"><ion-icon name="pricetags-outline" class="h-4 w-4" aria-hidden="true"></ion-icon>Sell</a>
                <a @click="mobileOpen = false" href="{{ route('about') }}" class="inline-flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium transition {{ request()->routeIs('about') ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800' }}"><ion-icon name="planet-outline" class="h-4 w-4" aria-hidden="true"></ion-icon>About Us</a>
                @auth
                    <a @click="mobileOpen = false" href="{{ route('saved-properties.index') }}" class="inline-flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium transition {{ request()->routeIs('saved-properties.*') ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800' }}"><ion-icon name="bookmark-outline" class="h-4 w-4" aria-hidden="true"></ion-icon>Saved</a>
                @endauth
            </nav>
